...
class IAPPNP(torch.autograd.Function):
...
class Net(nn.Module):
    ...
    def forward(self, x, adj, id, size, K, alpha, beta, theta, device):
        ...
        z_out = self.prop1.apply(x, adj, id, size, K, alpha, beta, theta, self.z_mem_tr, self.g_mem, device)
        self.z_mem_tr = self.z_mem_tr.to(device)
        self.z_mem_tr[id[:size]] = z_out[:size].clone().detach()
